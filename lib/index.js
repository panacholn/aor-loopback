    ...
    var convertRESTRequestToHTTP = function convertRESTRequestToHTTP(type, resource, params) {
        ...
        switch (type) {
            case _types.GET_LIST:
                {
                    ...
                    var _query = {};
                    _query['where'] = _extends({}, params.filter);
                    if (field) _query['order'] = [field + ' ' + order];
                    if (perPage > 0) {
                        _query['limit'] = perPage;
                        if (page >= 0) {
                            _query['skip'] = (page - 1) * perPage;
                        }
                    }
            ...
            case _types.GET_MANY_REFERENCE:
                {
                    ...
                    var _query3 = {};
                    _query3['where'] = _extends({}, params.filter);
                    _query3['where'][params.target] = params.id;
                    if (_field) _query3['order'] = [_field + ' ' + _order];
                    if (_perPage > 0) {
                        _query3['limit'] = _perPage;
                        if (_page >= 0) {
                            _query3['skip'] = (_page - 1) * _perPage;
                        }
                    }
                    url = apiUrl + '/' + resource + '?' + (0, _fetch.queryParameters)({ filter: JSON.stringify(_query3) });
                    break;
                }
            case _types.UPDATE:
                url = apiUrl + '/' + resource + '/' + params.id;
            ...
            case _types.DELETE_MANY:
                ...
                url = apiUrl + '/' + resource + '?' + (0, _fetch.queryParameters)({ filter: JSON.stringify(_query4) });
                options.method = 'DELETE';
                break;
            case _types.UPDATE_MANY:
                ...
                url = apiUrl + '/' + resource + '/update?' + (0, _fetch.queryParameters)({ where: JSON.stringify(query.where) });
                options.method = 'POST';
                options.body = JSON.stringify(params.data);
                break;
            default:
